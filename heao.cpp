...
void intialize() 
{
	...
	best_of_above = phy_memory_size;

	printf ("Enter hole_fitting algorithm (0=first fit, 1=best_fit): ");
	scanf ("%d", &hole_algo);
	return;
}
    	...
    	if (block_size <= (hl_end - hl_start))
		{
	  		...
	  		if (hole_algo != 0)
	    	{
	    		//printf ("best so far: %d\n", best_of_above);
	      		if ((hl_end - hl_start) <= best_of_above)
				{	
		  			best_of_above = hl_end - hl_start;
		  			best_start = hl_start;
		  			best_end = hl_end;
...
void f_block()//free block
{
    ...
    //if block_id found, free it and add memory to free(this is where reduction would be done)
  	if (current_block != NULL)
    {	prev_block->link = current_block->link;
      	remaining += current_block->end - current_block->start;
      	best_of_above += remaining;
      	free (current_block);
    }
  	else
    {
